...
{
	m_rShader = new CSulShaderCombineTex( getGeode(), pTexIn0, pTexIn1 );
}
...
{
	m_rShader = new CSulShaderCombineTex( getGeode(), texBuf0->getOut(), texBuf1->getOut() );
}

CSulPassCombineTex::CSulPassCombineTex( osg::Texture2D* pTexIn0, CSulTextureBuffer* texBuf1, osg::Texture2D* pTexOut ) :
CSulPass( pTexIn0, pTexOut, "CombineTex" )
{
	m_rShader = new CSulShaderCombineTex( getGeode(), pTexIn0, texBuf1->getOut() );
}
