...
REGISTER_OSGPLUGIN( bvh2, myReaderWriterBVH )

    myBvhMotionBuilder::myBvhMotionBuilder() : _drawingFlag(0) {}
    
	myBvhMotionBuilder::~myBvhMotionBuilder() {}

    void myBvhMotionBuilder::buildHierarchy( osgDB::Input& fr, int level, osgAnimation::Bone* parent )
    {
    ...
    void myBvhMotionBuilder::buildMotion( osgDB::Input& fr, osgAnimation::Animation* anim )
    {
        int i=0, frames=0;
    ...
    osg::Group* myBvhMotionBuilder::buildBVH( std::istream& stream, const osgDB::ReaderWriter::Options* options )
    {
        if ( options )
        {
...
#define XROTATION (0x08)
#define YROTATION (0x10)
#define ZROTATION (0x20)

	void myBvhMotionBuilder::alterChannel( std::string name, VEC_CHANNEL& value )
    {
		if      ( name=="Xposition" ) value.push_back( XPOSITION );
        else if ( name=="Yposition" ) value.push_back( YPOSITION );
        else if ( name=="Zposition" ) value.push_back( ZPOSITION );
        else if ( name=="Zrotation" ) value.push_back( ZROTATION );
        else if ( name=="Xrotation" ) value.push_back( XROTATION );
        else if ( name=="Yrotation" ) value.push_back( YROTATION );
    }

    void myBvhMotionBuilder::setKeyframe( osgDB::Input& fr, VEC_CHANNEL& vecChannels, double time,
                      ...
                      osgAnimation::QuatKeyframeContainer* rotKey )
    {
		...
		osg::Vec3 pos(0,0,0);
		osg::Matrix rotMat;
		osg::Matrix rotMatX, rotMatY, rotMatZ;
		int ch = 0;
		VEC_CHANNEL::iterator i = vecChannels.begin();
		VEC_CHANNEL::iterator ie = vecChannels.end();
		while ( i!=ie )
		{
			ch |= *i;

			if ( *i==XPOSITION ) fr.readSequence( pos[0] );
			if ( *i==YPOSITION ) fr.readSequence( pos[1] );
			if ( *i==ZPOSITION ) fr.readSequence( pos[2] );

			if ( *i==XROTATION )
			{
				float deg;
				fr.readSequence( deg );
				rotMatX = osg::Matrix::rotate(osg::DegreesToRadians(deg), osg::Vec3(1.0,0.0,0.0));
				rotMat *= osg::Matrix::rotate(osg::DegreesToRadians(deg), osg::Vec3(1.0,0.0,0.0));
			}
			...
			if ( *i==YROTATION )
			{
				float deg;
				fr.readSequence( deg );
				rotMatY = osg::Matrix::rotate(osg::DegreesToRadians(deg), osg::Vec3(1.0,0.0,0.0));
				rotMat *= osg::Matrix::rotate(osg::DegreesToRadians(deg), osg::Vec3(0.0,1.0,0.0));
			}
			...
			if ( *i==ZROTATION )
			{
				float deg;
				fr.readSequence( deg );
				rotMatZ = osg::Matrix::rotate(osg::DegreesToRadians(deg), osg::Vec3(1.0,0.0,0.0));
				rotMat *= osg::Matrix::rotate(osg::DegreesToRadians(deg), osg::Vec3(0.0,0.0,1.0));
			}
		...
		if ( ch&0x07 && posKey )
			posKey->push_back( osgAnimation::Vec3Keyframe(time, pos) );

        if ( ch&0x38 && rotKey )  
		{
			//rotMat = rotMatY * rotMatX * rotMatZ;

			osg::Quat quat;
			rotKey->push_back( osgAnimation::QuatKeyframe(time, rotMat.getRotate()) );
		}
    ...
    osg::ref_ptr<osg::Geode> myBvhMotionBuilder::createRefGeometry( osg::Vec3 p, double len )
    {
